...
def main() -> None:
    ...
    if mode == "fixture":
        ...
        query_params = {"per-page": str(max_records)}
        if query:
            query_params["search"] = query
        if filters:
            query_params["filter"] = ",".join(f"{key}:{value}" for key, value in sorted(filters.items()))
        url = "https://api.openalex.org/works?" + urllib.parse.urlencode(query_params)
        with urllib.request.urlopen(url, timeout=30) as response:
            payload = json.loads(response.read().decode("utf-8"))
        raw_dir = Path(os.environ["RWB_ARTIFACT_DIR"]) / "raw_responses"
        raw_dir.mkdir(parents=True, exist_ok=True)
        write_json(str(raw_dir / "openalex_query_1.json"), payload)
